...
import { ESTUDO_TIPO_OPTIONS } from "../../constants";
import {
  DangerActions,
  SubmitButton,
  fieldClass,
...
const ENTIDADE = "biblioteca_estudo";
const initialState: FormState = {};

export function EstudoModal({
  documento,
  catalogos,
  onClose,
}: {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label={documento ? "Editar estudo" : "Novo estudo"}
      ...
      >
        <header className="flex shrink-0 items-center justify-between border-b border-black/5 bg-white px-5 py-3">
          <h2 className="text-sm font-semibold text-[#24483F]">
            {documento ? "Editar estudo" : "Adicionar estudo"}
          </h2>
          <button
            type="button"
            onClick={close}
        ...
        <form action={formAction} className="flex-1 overflow-y-auto p-5" noValidate>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  type="text"
                  maxLength={200}
                  defaultValue={documento?.fonte ?? ""}
                  placeholder="Autor, publicação, canal..."
                  className={fieldClass}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  name="anotacoes"
                  rows={4}
                  defaultValue={documento?.anotacoes ?? ""}
                  className={fieldClass}
                />
              </div>

              <div>
                <label htmlFor="aplicacoes" className={labelClass}>
                  Possíveis aplicações na MIIRTUS
                </label>
                <textarea
                  id="aplicacoes"
                  name="aplicacoes"
                  rows={4}
                  defaultValue={documento?.aplicacoes ?? ""}
                  className={fieldClass}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  id="tipo"
                  name="tipo"
                  defaultValue={documento?.tipo ?? "artigo"}
                  className={fieldClass}
                >
          ...
          <div className="mt-6 flex items-center gap-3 border-t border-black/5 pt-4">
            <SubmitButton label={documento ? "Salvar alterações" : "Adicionar estudo"} />
            <button
              type="button"
              onClick={close}
