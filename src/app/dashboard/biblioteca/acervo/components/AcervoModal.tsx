...
import { ACERVO_TIPO_OPTIONS } from "../../constants";
import {
  DangerActions,
  SubmitButton,
  fieldClass,
...
const ENTIDADE = "biblioteca_acervo";
const initialState: FormState = {};

export function AcervoModal({
  documento,
  catalogos,
  onClose,
}: {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label={documento ? "Editar link" : "Novo link"}
        className={`relative flex max-h-[90vh] w-full max-w-[560px] flex-col overflow-hidden rounded-xl bg-[#F5F1E8] shadow-xl transition-all duration-200 ${
      ...
      >
        <header className="flex shrink-0 items-center justify-between border-b border-black/5 bg-white px-5 py-3">
          <h2 className="text-sm font-semibold text-[#24483F]">
            {documento ? "Editar link" : "Salvar link"}
          </h2>
          <button
            type="button"
            onClick={close}
        ...
        <form action={formAction} className="flex-1 overflow-y-auto p-5" noValidate>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                id="tipo"
                name="tipo"
                defaultValue={documento?.tipo ?? "link"}
                className={fieldClass}
              >
          ...
          <div className="mt-6 flex items-center gap-3 border-t border-black/5 pt-4">
            <SubmitButton label={documento ? "Salvar alterações" : "Salvar link"} />
            <button
              type="button"
              onClick={close}
